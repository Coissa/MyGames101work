...
{   
    ...
    float c1 = AB.cross(AP).z();
    float c2 = BC.cross(BP).z();
    float c3 = CA.cross(CP).z();
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    auto v = t.toVector4();
    int top = std::max({ (int)v[0].x(), (int)v[1].x(), (int)v[2].x() });
    int down = std::min({ (int)v[0].x(), (int)v[1].x(), (int)v[2].x() });
    int right = std::max({ (int)v[0].y(), (int)v[1].y(), (int)v[2].y() });
    int left = std::min({ (int)v[0].y(), (int)v[1].y(), (int)v[2].y() });
    int i = 0;
    int j = 0;
        ...
        for (j = left; j <= right; j++) {
            ...
            if (cnt != 0){
                ...
                if (z_interpolated <= depth_buf[get_index(i, j)]) {
                    ...
                    Eigen::Vector3f p(i, j, 0.0f);
                    Eigen::Vector3f& point_ref = p;

                    set_pixel(p, t.getColor() * cnt / 4);
                }
            }
        }
    }
